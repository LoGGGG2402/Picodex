...
import type { ResolvedCodexDesktopPaths } from "./codex-installation.js";
...
export async function loadCodexBundle(paths: ResolvedCodexDesktopPaths): Promise<CodexBundle> {
  const metadata = await loadCodexDesktopMetadata(paths);
...
export async function loadCodexDesktopMetadata(
  paths: Pick<ResolvedCodexDesktopPaths, "appPath" | "appAsarPath" | "infoPlistPath">,
): Promise<CodexDesktopMetadata> {
  const resolvedAppPath = resolve(paths.appPath);
  const appAsarPath = resolve(paths.appAsarPath);

  await stat(appAsarPath).catch(() => {
    throw new Error(`Codex app.asar not found at ${appAsarPath}`);
  });

  let infoPlist: CodexInfoPlist = {};
  if (paths.infoPlistPath) {
    const resolvedInfoPlistPath = resolve(paths.infoPlistPath);
    infoPlist = plist.parse(await readFile(resolvedInfoPlistPath, "utf8")) as CodexInfoPlist;
  }
